...
TITLE Joy Doovision Compiler - Thanks Tiger!
...
EXPLORER https://github.com/Ozymandias81/JoyDoovision
ECHO.
GOTO :EOF
...
START "JD Compile: 7Zip" /B /%4 /WAIT "%ProgramDirPath%tools\7za.exe" a -t%1 -mm=%2 -mx=%3 -x@"%ProgramDirPath%tools\7zExcludeListDir.txt" -xr@"%ProgramDirPath%tools\7zExcludeList.txt" "%ProgramDirPath%..\%projectName%.pk3" "%ProgramDirPath%*"
...
IF %featuresGit% NEQ True (
    ...
    IF "%projectName%" NEQ "JDpk3d" SET "projectName=JDpk3d"
    GOTO :EOF
)
...
REM Avoid redundancy
IF "%projectName%" NEQ "JDpk3d-%GitCommitHash%" SET "projectName=JDpk3d-%GitCommitHash%"
GOTO :EOF
